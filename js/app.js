...
function fIsCzechHoliday(date) {
    ...
    const easter = fGetEasterSunday(year);

    const goodFriday = fAddDays(easter, -2);
    const easterMonday = fAddDays(easter, 1);

    return fSameDate(date, goodFriday) || fSameDate(date, easterMonday);
}
window.fIsCzechHoliday = fIsCzechHoliday;

function fGetEasterSunday(year) {
    const a = year % 19;
...
window.fGetEasterSunday = fGetEasterSunday;


function fIsWeekend(date) {
    const d = date.getDay();
    return d === 0 || d === 6;
}
window.fIsWeekend = fIsWeekend;


function fAddDays(date, days) {
    const result = new Date(date);
    result.setDate(result.getDate() + days);
    return result;
}
window.fAddDays = fAddDays;

function fSameDate(a, b) {
    return (
...
window.fSameDate = fSameDate;

function fGetEng3Weekday(date) {

    const lstDays = ["sun","mon","tue","wed","thu","fri","sat"];

    return lstDays[date.getDay()];
}
